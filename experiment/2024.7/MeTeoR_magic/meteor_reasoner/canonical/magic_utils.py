...
def find_periods_with_query(CR,fact):
...
def find_periods_with_query_list(CR,fact_list):
    left_period, left_len = defaultdict(list), 0
    right_period, right_len = defaultdict(list), 0
    number_mat = 0
    ...
    while True:
        ...
        for fact in fact_list:
            if entail(fact,CR.D) == True:
                ...
                print("Entailment time:", execution_time, "seconds")
                print("Entailment: True!")
                with open(f"./iTemporal_baseline_results/{fact}_inside.txt", 'w') as f:
                    f.write(f"entailed with no cr!\nquery_time is(without CR):{execution_time}")
